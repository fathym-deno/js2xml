...
const cases = [
  ...
  },
  {
    ...
    js2: {
      "elements": [{
        ...
        "name": "go",
        "instruction": "there",
      }, { "type": "instruction", "name": "come", "instruction": "here" }],
    },
  },
  {
  ...
  },
  {
    ...
    js1: { "_comment": [" \t Hello \t ", " \t World \t "] },
    js2: {
      "elements": [{ "type": "comment", "comment": " \t Hello \t " }, {
        "type": "comment",
        "comment": " \t World \t ",
      }],
    },
  ...
  },
  {
    ...
    js1: { "_cdata": [" \t data", '< > " and & \t '] },
    js2: {
      "elements": [{ "type": "cdata", "cdata": " \t data" }, {
        "type": "cdata",
        "cdata": '< > " and & \t ',
      }],
    },
...
export default (direction, options) => {
  ...
  function applyOptions(obj, pathKey) {
    ...
    if (obj instanceof Array) {
      obj = obj.filter(function (el) {
        return !(options.ignoreText && el.type === "text" ||
          options.ignoreComment && el.type === "comment" ||
          options.ignoreCdata && el.type === "cdata" ||
          options.ignoreDoctype && el.type === "doctype" ||
          options.ignoreDeclaration && el.type === "declaration" ||
          options.ignoreInstruction && el.type === "instruction");
      }).map(function (el) {
      ...
      for (key in obj) {
        ...
        if (
          ...
          (Array.isArray(options.alwaysArray)
            ? options.alwaysArray.indexOf(key) !== -1
            : options.alwaysArray) &&
        ...
        if (
          options.addParent /*&& key.indexOf('declaration') === -1*/ &&
          ...
          if (obj[key] instanceof Array) {
            ...
            } else obj[key].parent = obj;
          }
        }
        if (
          options.ignoreText && key === "_text" ||
          options.ignoreComment && key === "_comment" ||
          options.ignoreCdata && key === "_cdata" ||
          options.ignoreDoctype && key === "_doctype" ||
          options.ignoreDeclaration &&
            (key === "_declaration" || key === "declaration") ||
          options.ignoreInstruction && key === "_instruction"
        ) {
          delete obj[key];
        }
      }
      ...
      if (
        !options.compact && options.alwaysChildren && obj.type === "element" &&
        !obj.elements
      ) {
        obj.elements = [];
      }
    ...
    function manipulate(x, fullKey) {
      ...
      if (typeof x === "object") {
        ...
        x = applyValueCallbacks(
          x,
          fullKey.split(".").pop(),
          fullKey.split(".")[fullKey.split(".").length - 2] || "",
        );
  ...
  function applyNameCallbacks(obj, key, parentKey) {
    if (
      "instructionNameFn" in options &&
        (options.compact && parentKey === "_instruction" ||
          !options.compact && obj.type === "instruction") ||
      "elementNameFn" in options &&
        (options.compact && key.indexOf("_") < 0 &&
            parentKey !== "_attributes" && parentKey !== "_instruction" ||
          !options.compact && obj.type === "element")
    ) {
      if (options.compact) {
        const temp = obj[key];
        delete obj[key];
        key = "elementNameFn" in options
          ? options.elementNameFn(key)
          : options.instructionNameFn(key);
        obj[key] = temp;
      } else {
        obj.name = "elementNameFn" in options
          ? options.elementNameFn(obj.name)
          : options.instructionNameFn(obj.name);
      }
    }
    return key;
  }
  function applyAttributesCallback(obj, key, parentKey) {
    ...
    if (
      ("attributeNameFn" in options || "attributeValueFn" in options) &&
      (parentKey === "_attributes" || parentKey === "attributes")
    ) {
    ...
    if (
      "attributesFn" in options &&
      (key === "_attributes" || key === "attributes")
    ) {
  ...
  function applyValueCallbacks(value, key, parentKey) {
    ...
    for (fn in options) {
      if (fn.match(/Fn$/) && !fn.match(/NameFn$/)) {
        const callbackName = (options.compact ? "_" : "") +
          fn.replace("Fn", "");
  ...
  for (i = 0; i < cases.length; ++i) {
    ...
    if (direction === "xml2js") {
      ...
      if (
        !("spaces" in options) || options.spaces === 0 ||
        typeof options.spaces === "boolean"
      ...
      if (options.fullTagEmptyElement) {
        tests[i].xml = tests[i].xml.replace("<a/>", "<a></a>").replace(
          "<b/>",
          "<b></b>",
        ).replace("<c/>", "<c></c>").replace("/>", "></a>");
      }
    }
  }
